...
UILayer::UILayer(InterfaceFramework& Iframe, UINT nTextBlocks)
{
    m_fWidth = static_cast<float>(Iframe.m_nWndClientWidth);
    m_fHeight = static_cast<float>(Iframe.m_nWndClientHeight);
    m_nRenderTargets = Iframe.GetSwapChainBuffersNum();
    m_ppd3d11WrappedRenderTargets = new ID3D11Resource * [m_nRenderTargets];
    m_ppd2dRenderTargets = new ID2D1Bitmap1 * [m_nRenderTargets];

    m_nTextBlocks = nTextBlocks;
    m_pTextBlocks = new TextBlock[nTextBlocks];

    InitializeDevice(&Iframe.GetDevice(), &Iframe.GetCommandQueue(), Iframe.GetRenderTargetBuffers());
   
    for (int n = 0; n < m_nRenderTargets; ++n)
    {
        Iframe.ResetCommand();
        Iframe.SynchronizeResourceTransition(D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
        Iframe.ExecuteCommand();
        InitializeRenderTarget(Iframe.GetSwapChainBufferIndex());
        Iframe.GetSwapChain().Present(0, 0);
        Iframe.MoveToNextFrame();
    }
}
UILayer::~UILayer()
{
...
void UILayer::InitializeRenderTarget(UINT nFrame)
{
...
//-------------------------------------------------------------------------------
UILayerLobby_1::UILayerLobby_1(InterfaceFramework& Iframe, UINT nTextBlocks) : UILayer(Iframe, nTextBlocks)
{
    Build();
}
UILayerLobby_1::~UILayerLobby_1()
{
}

void UILayerLobby_1::Render(UINT nFrame)
{
    UILayer::Render(nFrame);
}
void UILayerLobby_1::Build()
{
    ID2D1SolidColorBrush* pd2dBrush;
    IDWriteTextFormat* pdwTextFormat;
    D2D1_RECT_F d2dRect;
...
//-------------------------------------------------------------------------------
UILayerGame_1::UILayerGame_1(InterfaceFramework& Iframe, UINT nTextBlocks) : UILayer(Iframe, nTextBlocks)
{
    Build();
}
UILayerGame_1::~UILayerGame_1()
{
}

void UILayerGame_1::Render(UINT nFrame)
{
    UILayer::Render(nFrame);
}
void UILayerGame_1::Build()
{
    ID2D1SolidColorBrush* pd2dBrush;
    IDWriteTextFormat* pdwTextFormat;
    D2D1_RECT_F d2dRect;
