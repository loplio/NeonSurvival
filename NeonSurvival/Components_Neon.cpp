			...
			if (m_pPlayer->GetReadyFire())
			{
				if (((PistolBulletTexturedObjects*)m_ppShaders[i])->m_fCoolTime <= 0.0f && !m_pPlayer->IsDash)
				{
					...
					if (m_pPlayer.get()->bSelectedObject)
						pObjectsShader->AppendBullet(startLocation, rayDirection, 0, true, m_pPlayer.get()->fDistanceAtObject); //총알
					else
						pObjectsShader->AppendBullet(startLocation, rayDirection, 0, true); //총알
