...
{
	// StandardShader's RtvFormats (Base[FlipBuffer], Color, wNormal, Texture, Emissive, Shading, RoughMetal)
	DXGI_FORMAT pdxgiRtvFormats[BACKBUFFER_NUM_INCLUDE_FLIP] = {
	...
	if (GetNumRenderTargets() > 1)
		for (UINT i = 0; i < GetNumRenderTargets(); i++) m_d3dPipelineStateDesc.RTVFormats[i] = pdxgiRtvFormats[i];
	else
	...
	HRESULT hResult = pd3dDevice->CreateGraphicsPipelineState(&m_d3dPipelineStateDesc, __uuidof(ID3D12PipelineState), (void**)&m_ppd3dPipelineStates[m_nCurrentPipelineState++]);
...
void CComputeShader::CreateComputePipelineState(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dRootSignature, UINT cxThreadGroups, UINT cyThreadGroups, UINT czThreadGroups)
{
	...
	HRESULT hResult = pd3dDevice->CreateComputePipelineState(&d3dPipelineStateDesc, __uuidof(ID3D12PipelineState), (void**)&m_ppd3dPipelineStates[m_nCurrentPipelineState++]);
...
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSWireFrame", "vs_5_1", &m_pd3dVertexShaderBlob);
}
D3D12_SHADER_BYTECODE CBoundingBoxShader::CreatePixelShader()
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSWireFrame", "ps_5_1", &m_pd3dPixelShaderBlob);
}
...
//-------------------------------------------------------------------------------
/*	CPathShader : public Shader												   */
//-------------------------------------------------------------------------------
CPathShader::CPathShader()
{
}
CPathShader::~CPathShader()
{
...
D3D12_INPUT_LAYOUT_DESC CPathShader::CreateInputLayout()
{
	UINT nInputElementDescs = 1;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_SHADER_BYTECODE CPathShader::CreateVertexShader()
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSPathFrame", "vs_5_1", &m_pd3dVertexShaderBlob);
}
D3D12_SHADER_BYTECODE CPathShader::CreatePixelShader()
{
	return CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSPathFrame", "ps_5_1", &m_pd3dPixelShaderBlob);
}
void CPathShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
...
//-------------------------------------------------------------------------------
/*	CPostProcessingShader : public CShader									   */
//-------------------------------------------------------------------------------
CPostProcessingShader::CPostProcessingShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, UINT nResources, DXGI_FORMAT* pdxgiFormats, UINT nWidth, UINT nHeight, D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle)
{
	m_pTexture = new CTexture(nResources, RESOURCE_TEXTURE2D_ARRAY, 0, 1);
	...
	for (UINT i = 0; i < nResources; i++)
	{
		...
		m_pTexture->CreateTexture(pd3dDevice, pd3dCommandList, nWidth, nHeight, pdxgiFormats[i], D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON, &d3dClearValue, RESOURCE_TEXTURE2D_ARRAY, i);
	}
...
{
	return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSPostProcessing", "vs_5_1", &m_pd3dVertexShaderBlob));
}
D3D12_SHADER_BYTECODE CPostProcessingShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSPostProcessing", "ps_5_1", &m_pd3dVertexShaderBlob));
}

void CPostProcessingShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
...
void CPostProcessingShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	pd3dCommandList->SetGraphicsRootConstantBufferView(ROOT_PARAMETER_DRAW_OPTION, d3dGpuVirtualAddress);
}
...
void CStandardShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	SetNumRenderTargets(BACKBUFFER_NUM_INCLUDE_FLIP);
...
//-------------------------------------------------------------------------------
/*	CSkinnedAnimationStandardShader : public CStandardShader				   */
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
/*	CSkinnedAnimationObjectsShader : public CStandardShader				   */
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
/*	CParticleShader : public CBillboardShader								   */
//-------------------------------------------------------------------------------
...
{
	return((m_nCurrentPipelineState == 0) ? 0 : 1);
}
DXGI_FORMAT CParticleShader::GetRTVFormat()
{
	return((m_nCurrentPipelineState == 0) ? DXGI_FORMAT_UNKNOWN : DXGI_FORMAT_R8G8B8A8_UNORM);
}
...
D3D12_SHADER_BYTECODE CParticleShader::CreateVertexShader()
{
	if (m_nCurrentPipelineState == 0)
		return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSParticleStreamOutput", "vs_5_1", &m_pd3dVertexShaderBlob));
	else
		return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSParticleDraw", "vs_5_1", &m_pd3dVertexShaderBlob));
}
D3D12_SHADER_BYTECODE CParticleShader::CreateGeometryShader()
{
	if (m_nCurrentPipelineState == 0)
		return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "GSParticleStreamOutput", "gs_5_1", &m_pd3dGeometryShaderBlob));
	else
		return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "GSParticleDraw", "gs_5_1", &m_pd3dGeometryShaderBlob));
}
D3D12_SHADER_BYTECODE CParticleShader::CreatePixelShader()
{
	if (m_nCurrentPipelineState == 0)
		...
		return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSParticleDraw", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void CParticleShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 2;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];

	CShader::CreateGraphicsPipelineState(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature); // Pipeline0 - Stream Output Pipeline State
	CShader::CreateGraphicsPipelineState(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature); // Pipeline1 - Draw Pipeline State
}

//-------------------------------------------------------------------------------
/*	CBulletShader : public CBillboardShader									   */
//-------------------------------------------------------------------------------
...
{
	return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "VSBullet", "vs_5_1", &m_pd3dVertexShaderBlob));
}
D3D12_SHADER_BYTECODE CBulletShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile((wchar_t*)L"Shaders.hlsl", "PSBullet", "ps_5_1", &m_pd3dPixelShaderBlob));
}
...
void CBulletShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
	CShader::CreateGraphicsPipelineState(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
