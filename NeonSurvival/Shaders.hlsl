...
VS_TEXTURED_OUTPUT VSBlend(VS_TEXTURED_INPUT input)
{
	VS_TEXTURED_OUTPUT output;
...
float4 PSBlend(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = gtxtTexture.Sample(gssWrap, input.uv);
	cColor.a = 0.6f;
...
struct VS_SKINNED_STANDARD_INST_INPUT
{
	float3 position : POSITION;
	float2 uv : TEXCOORD;
	float3 normal : NORMAL;
...
VS_STANDARD_OUTPUT VSSkinnedAnimationStandard_Inst(VS_SKINNED_STANDARD_INST_INPUT input)
{
	VS_STANDARD_OUTPUT output;
	...
	mtxVertexToBoneWorld[3][0] += input.transform[3][0];
	mtxVertexToBoneWorld[3][1] += input.transform[3][1];
	mtxVertexToBoneWorld[3][2] += input.transform[3][2];
	//mtxVertexToBoneWorld = mul(input.transform, mtxVertexToBoneWorld);
	output.positionW = mul(float4(input.position, 1.0f), mtxVertexToBoneWorld).xyz;

	//output.positionW = mul(float4(output.positionW, 1.0f), input.transform).xyz;
	output.normalW = mul(input.normal, (float3x3)mtxVertexToBoneWorld).xyz;
	output.tangentW = mul(input.tangent, (float3x3)mtxVertexToBoneWorld).xyz;
	output.bitangentW = mul(input.bitangent, (float3x3)mtxVertexToBoneWorld).xyz;
